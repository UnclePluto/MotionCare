...
以下决策覆盖本文原难度及分类切换、声音成对数量的约定，历史正文保留：

- 图案顺序的候选数量和每题序列长度固定为简单 3、中等 4、困难 5；每项完整展示 2 秒，保留项间 500ms 过渡。
- 分类转换所有档位只判断物品类别，分别提供 3/4/5 个唯一选项，且恰有一个正确答案。每题提示为“请判断彩图中的物体属于哪个类别，并选出正确的选项”。不生成颜色题或场景题。
- 声音辨别分别提供 3/4/5 张卡片，保留同类别不同声音的干扰项和具体 soundId 判定。奇数张使用成对类别加一个单张类别；所有卡片先试听，目标来自本轮卡片且只有一个正确选项。
- 拼图保持 4/6/9 块。各游戏未在 Excel 定义的作答时限与拼图预览时长保持现有配置。
- 医生开方、患者开始前查看说明与降低难度、四项原因单选及记录方式，遵循同日更新的 `2026-05-16-wechat-miniapp-real-games-design.md`。
- 长题目自然换行；选项等宽、间距稳定，奇数末项不拉伸；降低原因纵向排列。
