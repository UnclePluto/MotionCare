...
以下决策覆盖本文原难度建议与允许患者提高难度的约定，历史正文保留：

- 按用户提供的《游戏训练项目表.xlsx》配置三档难度。颜色顺序的候选数量和每题序列长度固定为 3/4/5；每项展示 2 秒，保留项间 500ms 过渡。反应抑制保持 4/6/9 个数字。
- 医生开方时可选择简单、中等、困难并查看对应规则；新开游戏默认简单，已有游戏回显处方难度。
- 患者开始前查看设定难度、本次难度和规则，只能选择比处方低的档位，也可恢复处方设定；只影响本次训练。
- 降低原因四项单选必选：切换速度问题、选项个数问题、思考时间问题、其他。其他允许补充说明。原因只记录反馈，不单独改变速度、选项数或作答时限。
- 沿用实际难度、处方难度及调整原因的现有记录与上传字段。Excel 未定义的作答时限沿用现有各档配置，整场时长仍由处方决定。
- 小程序沿用中性术语约定，将医生展示为“指导老师”、处方展示为“运动计划”。
