...
> 发布记录（2026-09-08, codex）：`c58d7aa` 两阶段生产流水线均成功，正式 API 的 23 项签名素材验收通过，微信开发版 7.0.4 已上传；普通 H5 端到端受既有 `Taro.login` 限制，真机验收、平台审核与正式上架待完成。
...
### Task 7 实际发布状态（2026-09-08）

- 第一阶段：[生产流水线 34181288314](https://github.com/UnclePluto/MotionCare/actions/runs/34181288314) 成功，SHA `c58d7aa`；后端与管理端更新，患者 H5 按门禁沿用旧镜像。
- 正式 API + 真实 CDN 验收：23/23 项正文状态、媒体类型、字节数及 SHA-256 通过，匿名、篡改、过期访问均被拒绝。
- `TARO_APP_ASSET_BASE_URL` 已设为正式素材基址；第二阶段 [生产流水线 34182242505](https://github.com/UnclePluto/MotionCare/actions/runs/34182242505) 成功，患者 H5 镜像实际重建并部署，旧镜像复用分支跳过。
- 微信 CLI 使用同一已验证生产 `dist` 上传 7.0.4，描述“小游戏难度及私有素材加载优化”，业务结果为 `✔ upload`。当前状态仅为开发版上传，未提交平台审核、未正式上架。
- 用户再次确认 `cdn.whestsun.com` 已列入 `downloadFile` 合法域名。普通 Chrome 可打开新 H5 并进入绑定页，但在绑定码输入前受既有 `Taro.login` 限制，无法完成 8888 全链路；未注入伪登录。真机图片下载和语音播放仍待验，因此 Steps 4–6 保持未勾选，spec/索引保持 implementing。
