...
**执行记录（executing-plans）：** Task 1–4 已于 **`241fc51`**（`docs(plan): drop-batch后收口执行计划并对齐spec与验收`）落地。Task 5 于后续会话复验：`pytest` **43 passed**；`npm run test` **12 passed**；`npm run build` **exit 0**；`npm run lint` **0 errors**（1 warning：`AuthContext.tsx` react-refresh）。Task 3 字符串审计：应用代码（非 migrations）**零命中**。
...
- [x] **Task 1** — design spec 已与 `main` 对齐（见 `241fc51` 及当前文件树）。  
- [x] **Task 2** — `drop-batch-concept.md` 状态与第四节 `[x]`（见 `241fc51`）。  
- [x] **Task 3** — 应用代码无批次残留（仅 `migrations/`）。  
- [x] **Task 4** — `ProjectGroupingBoard.test.tsx` 已合并（`MemoryRouter` + `within` 患者池卡片）。  
- [x] **Task 5** — 见文首 **执行记录** 中的命令与结果摘要。
