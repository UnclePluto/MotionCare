...
- 真实视频硬验收：输入人工真值必须固定为 90；报告必须且只能包含 `all_frames`，其 `sample_fps` 显式为 `null`，解码帧数与推理帧数为相等的非负整数；计数等于 90 且字段自洽；总耗时不超过 600 秒；峰值 RSS 低于 1.5 GiB；Swap 使用为零。
...
- [x] **Step 8: 用 `6d0c2e9` 拒绝缺少全帧采样证据、解码与推理帧数不相等，以及人工真值不是 90 的报告和命令输入**
