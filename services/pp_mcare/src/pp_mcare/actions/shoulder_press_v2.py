...
from .base import ActionAnalysisError, AnalysisResult, PoseFrame
...
def _angle(
    ...
    if denominator <= 0:
        return 0.0
    dot_product = sum(a * b for a, b in zip(first_vector, third_vector, strict=True))
...
def _measurement(frame: dict[str, Any], side: str) -> SideMeasurement | None:
    try:
        ...
        shoulder_x, shoulder_y, shoulder_score = _point(keypoints, f"{side}_shoulder")
...
class SideEventDetector:
    ...
    def observe(
        ...
        *,
        ...
        if self.phase == "seeking":
            if not _below_threshold(smoothed_lift - self.trough_lift, RISE_HYSTERESIS):
                self.phase = "rising"
    ...
    def _count_candidate_frame(self, measurement: SideMeasurement, opposite_valid: bool) -> None:
        self.candidate_total_frames += 1
        self.candidate_reliable_frames += int(measurement.score >= RELIABLE_SCORE_THRESHOLD)
        self.candidate_opposite_reliable_frames += int(opposite_valid)
    ...
    def _complete_event(self, end_ms: int) -> SideEvent | None:
        ...
        if _below_threshold(prominence, MIN_EVENT_PROMINENCE) or not far_enough_from_previous:
            return None
        ...
        event = SideEvent(
            ...
            opposite_coverage_ratio=(self.candidate_opposite_reliable_frames / total_frames),
        )
...
def _event_flags(anchor_event: SideEvent, events: tuple[SideEvent, ...]) -> list[str]:
    flags = []
    reliable_events = tuple(
        event for event in events if event.coverage_ratio >= STANDARD_MIN_COVERAGE_RATIO
    )
    if any(
...
def _rep_detail(*, anchor_event: SideEvent, matched_event: SideEvent | None) -> dict[str, Any]:
    events = (anchor_event, matched_event) if matched_event else (anchor_event,)
...
def _merge_events(
    ...
    left_average_coverage = sum(event.coverage_ratio for event in left_events) / max(
        len(left_events), 1
    )
    right_average_coverage = sum(event.coverage_ratio for event in right_events) / max(
        len(right_events), 1
    )
    if len(right_events) > len(left_events) or (
        len(right_events) == len(left_events) and right_average_coverage > left_average_coverage
    ):
...
def analyze_shoulder_press_keypoints_v2(frames: Iterable[dict]) -> dict[str, Any]:
    ...
    for frame in frames:
        ...
        measurements = {side: _measurement(frame, side) for side in ("left", "right")}
        if any(measurements.values()):
            measurable_frames += 1
        reliable = {
            side: measurement is not None and measurement.score >= RELIABLE_SCORE_THRESHOLD
        ...
        for side, opposite in (("left", "right"), ("right", "left")):
            ...
            if measurement is None:
                detectors[side].observe_missing(timestamp_ms, opposite_valid=reliable[opposite])
                continue
            event = detectors[side].observe(measurement, opposite_valid=reliable[opposite])
            if event is not None:
                events[side].append(event)

    if processed_frames == 0:
        raise ActionAnalysisError("关键点流没有可分析帧")
    if measurable_frames == 0:
        raise ActionAnalysisError("全程没有可用的人体关键点")
    ...
    agreement_ratio = bilateral_count / max(len(events["left"]), len(events["right"]), 1)
...
class ShoulderPressV2Plugin:
    ...
    def analyze(self, frames: Iterable[PoseFrame]) -> AnalysisResult:
        def legacy_frames() -> Iterable[dict[str, Any]]:
            for frame in frames:
                yield {
                    "timestamp_ms": frame.timestamp_ms,
                    "keypoints": {
                        name: {"x": point[0], "y": point[1], "score": point[2]}
                        for name, point in frame.named_keypoints.items()
                    },
                }

        payload = analyze_shoulder_press_keypoints_v2(legacy_frames())
        return AnalysisResult(counts=validate_counts(payload), payload=payload)
