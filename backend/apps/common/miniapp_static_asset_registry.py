...
REGISTERED_STATIC_ASSET_MANIFESTS: dict[str, Path] = {
    REST_ASSET_VERSION: MANIFEST_ROOT / f"{REST_ASSET_VERSION}.json",
...
def _validate_registered_manifest(payload: object, version: str) -> dict:
    ...
    specs = ALL_ASSET_SPECS if version == REST_ASSET_VERSION else CANONICAL_ASSET_SPECS
    ...
    if len(set(keys)) != len(keys) or set(keys) != set(specs):
        raise ValueError("已登记固定素材清单 key 无效")

    for entry in entries:
        key = entry["key"]
        expected_kind, expected_content_type, extension = specs[key]
