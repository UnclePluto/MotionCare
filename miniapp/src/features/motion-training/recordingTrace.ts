...
export const RECORDING_BUILD = '7.0.14'
...
export function createRecordingTrace(now = Date.now) {
  const startedAt = now()
  const events: Entry[] = []
  let group = 0
  let generation = 0
  ...
  return {
    ...
    export() { return JSON.stringify({ schema: 1, build: RECORDING_BUILD, startedAt: new Date(startedAt).toISOString(), events }) },
