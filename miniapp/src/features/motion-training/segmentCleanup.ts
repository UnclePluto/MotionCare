...
export async function retryAbandonedMotionTrainingFiles(
  storage: StorageLike,
  releaseFile: (path: string) => Promise<boolean>,
  isActive: () => boolean,
  ...
  for (const path of abandonedPaths(storage)) {
    ...
    let removed = false
    try { removed = await releaseFile(path) } catch { /* Retain for the next preflight. */ }
    if (!isActive()) return false
    if (!removed) continue
    // A concurrent completion may have added other paths while deletion was pending.
    const remaining = abandonedPaths(storage).filter(item => item !== path)
    if (remaining.length) storage.setStorageSync(ABANDONED_MOTION_TRAINING_FILES_KEY, { paths: remaining, recordedAt: Date.now() })
...
export async function cleanupUploadedMotionTrainingSegments(input: {
  ...
  releaseFile: (segment: CompressedMotionTrainingSegment) => Promise<boolean>
}): Promise<void> {
  ...
  for (const candidate of initial.segments) {
    ...
    let removed = false
    try { removed = await input.releaseFile(segment) } catch { /* Keep the path and occupied bytes for retry. */ }
    if (!removed) continue
    ...
    try {
      input.saveSession({ ...latest, segments: latest.segments.map(current => (
        ...
        current.uploadState === 'uploaded' && current.savedFilePath === segment.savedFilePath
