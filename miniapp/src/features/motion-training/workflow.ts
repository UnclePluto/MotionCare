...
export type MotionTrainingUploadPhase = 'session' | 'status' | 'upload' | 'finalize' | 'credential' | 'complete'

export type MotionTrainingUploadEvent = {
  phase?: MotionTrainingUploadPhase
  index: number
...
export type PendingSegmentUploadDependencies = {
  ...
  saveSession: (session: PendingMotionTrainingSession) => void
...
type LegacyWorkflowDependencies = {
  ...
  savePending?: (pending: PendingMotionTrainingUpload) => void
}

const REMOTE_SEGMENTS_ERROR_MESSAGE = '服务端分段状态不一致，请重新上传'

export function motionTrainingUploadErrorMessage(error: unknown): string {
...
export async function runPendingSegmentUploads(
  ...
  onProgress: (event: MotionTrainingUploadEvent) => void
): Promise<PendingMotionTrainingSession> {
  let session: PendingMotionTrainingSession = { ...initialSession, lastError: undefined }

  try {
    if (session.segments.some((segment) => !isCompressedMotionTrainingSegment(segment))) {
    ...
    const uploading = session.segments.find((segment) => (
      isCompressedMotionTrainingSegment(segment) && segment.uploadState === 'uploading'
    ))
    if (uploading) {
    ...
    persist({ ...session, lastError: motionTrainingUploadErrorMessage(error) }, dependencies)
    throw error
  }
}

export async function runMotionTrainingUploadWorkflow(
  initialPending: PendingMotionTrainingUpload,
  dependencies: LegacyWorkflowDependencies,
  onEvent: (event: { phase: MotionTrainingUploadPhase; progress: number }) => void
): Promise<PendingMotionTrainingUpload> {
  const firstSegment = initialPending.segments[0]
  if (
    initialPending.segments.length !== 1 ||
    !firstSegment ||
    !isCompressedMotionTrainingSegment(firstSegment) ||
  ...
  if (!videoId) {
    ...
    const trainingStartedAt = requireMotionTrainingStartedAt(initialPending)
  ...
  const pendingSession: PendingMotionTrainingSession = {
    clientSessionId: initialPending.clientSessionId,
    videoId,
    actionId: initialPending.actionId,
    trainingDate: initialPending.trainingDate,
  ...
  const toLegacyPending = (session: PendingMotionTrainingSession): PendingMotionTrainingUpload => {
    const segment = session.segments[0]
    if (!segment || !isCompressedMotionTrainingSegment(segment)) {
      throw new Error('录像上传信息不完整，请重新开始')
    }
    return {
      ...initialPending,
