...
const taroHarness = vi.hoisted(() => {
  ...
  const taroMock = {
    ...
    getDeviceInfo: vi.fn(() => ({ platform: 'ios', system: 'iOS 26.6.1', model: 'iPhone 13' })),
    getAppBaseInfo: vi.fn(() => ({ version: '8.0.78', SDKVersion: '3.17.0' })),
...
const motionInstructionAudioHarness = vi.hoisted(() => ({
  play: vi.fn(async (_src: string) => true),
  stop: vi.fn(),
  dispose: vi.fn(),
...
vi.mock('../../features/motion-training/directUpload', () => ({ uploadDirectVideo: directUploadMock }))
...
describe('shoulder press pages', () => {
  ...
  it('reloads preview when the same mounted component receives another action id', async () => {
    requestMock
      .mockResolvedValueOnce(PRESCRIPTION)
      .mockResolvedValueOnce(prescriptionForMotionAction(MOTION_ACTION_CASES[2]))
    const preview = renderPage(ShoulderPressPreviewPage)
    await flushPromises()
    preview.rerender()
    expect(findFirstByType(preview.element, 'Video').props.src)
      .toBe(PRESCRIPTION.actions[0].video_url)

    taroHarness.routerParams.actionId = String(MOTION_ACTION_CASES[2].id)
    preview.rerender()
    await flushPromises()
    preview.rerender()

    expect(findFirstByType(preview.element, 'Video').props.src)
      .toBe(MOTION_ACTION_CASES[2].videoUrl)
  })
  ...
  it('refreshes a failed overlay URL once and clears the failure when the URL changes', async () => {
    ...
    let overlayProps = {
      ...
      expectedDurationSeconds: 180,
      started: true,
      onVideoError: refresh
    }
    const OverlayHost = () => ShoulderPressTrainingOverlay(overlayProps)
    const overlay = renderPage(OverlayHost)
    ...
    overlayProps = {
      ...overlayProps,
      videoUrl: 'https://cdn.example.com/refreshed.mp4'
    }
    overlay.rerender()
    overlay.rerender()
    expect(findFirstByType(overlay.element, 'Video').props.src)
      .toBe('https://cdn.example.com/refreshed.mp4')
  ...
  it('creates and persists the server session before constructing or starting the recorder', async () => {
    const created = deferred<{ video_id: number; status: 'recording'; uploaded_segments: number[] }>()
    apiMocks.createVideoSession.mockReturnValueOnce(created.promise)
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises()
    page.rerender()
    initializeCamera(page.element)
    ...
    created.resolve({ video_id: 77, status: 'recording', uploaded_segments: [] })
    await flushPromises(30)

    expect(taroHarness.storage.get(PENDING_SHOULDER_PRESS_SESSION_KEY)).toMatchObject({
      videoId: 77,
      trainingStartedAt: expect.any(String),
      segments: []
    })
    expect(recorderHarness.instances[0].start).toHaveBeenCalledTimes(1)
  ...
  it('does not construct a recorder, start timers, or persist fragments when session creation fails', async () => {
    apiMocks.createVideoSession.mockRejectedValueOnce(new Error('服务端会话暂时不可用'))
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises()
    page.rerender()
    initializeCamera(page.element)
    ...
    expect(textContent(page.element)).toContain('服务端会话暂时不可用')
    expect(findTrainingOverlay(page.element).props.started).toBe(false)
  })

  it('refreshes the real camera overlay without touching recorder or upload state', async () => {
    requestMock
      .mockResolvedValueOnce(PRESCRIPTION)
      .mockResolvedValueOnce({
        ...PRESCRIPTION,
        actions: [{
          ...PRESCRIPTION.actions[0],
          video_url: 'https://cdn.example.com/camera-refreshed.mp4'
        }]
      })
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises()
    ...
    await findTrainingOverlay(page.element).props.onVideoError?.()
    await flushPromises(20)
    page.rerender()

    expect(findTrainingOverlay(page.element).props.videoUrl)
      .toBe('https://cdn.example.com/camera-refreshed.mp4')
    expect(recorderHarness.instances).toHaveLength(0)
  ...
  it('keeps recording and segment recovery active when the real overlay refresh fails', async () => {
    requestMock
      .mockResolvedValueOnce(PRESCRIPTION)
      .mockRejectedValueOnce(new Error('video refresh failed'))
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises()
    page.rerender()
    initializeCamera(page.element)
    ...
    const refresh = findTrainingOverlay(page.element).props.onVideoError
    if (!refresh) throw new Error('真实视频刷新回调缺失')
    await expect(refresh()).rejects.toThrow('video refresh failed')
    ...
    await recorder.options.onSegment('wxfile://temp/after-refresh-failure.mp4', 5_000)
    await flushPromises(20)
  ...
  it('keeps the pre-created server session but records no fragment when the page hides during recorder start', async () => {
  ...
  it('freezes effective time while a segment waits for metadata and resumes at native start', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-09-24T10:00:00Z'))
    recorderHarness.useReal = true
    const starts: Array<{ success?: () => void; timeoutCallback?: (result: { tempVideoPath: string }) => void }> = []
    const camera = { startRecord: vi.fn(options => { starts.push(options); options.success?.() }), stopRecord: vi.fn() }
    const originalCamera = taroHarness.taroMock.createCameraContext.getMockImplementation()!
    taroHarness.taroMock.createCameraContext.mockReturnValue(camera)
    const metadata = deferred<{ duration: number; size: number; width: number; height: number }>()
    taroHarness.taroMock.getVideoInfo.mockReturnValueOnce(metadata.promise)
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises()
    page.rerender(); initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始训练')
    await flushPromises(); page.rerender()
    await vi.advanceTimersByTimeAsync(60_000)
    starts[0].timeoutCallback?.({ tempVideoPath: 'wxfile://temp/slow-metadata.mp4' })
    await flushPromises()
    await vi.advanceTimersByTimeAsync(5_000)
    page.rerender()
    expect(starts).toHaveLength(1)
    expect(findTrainingOverlay(page.element).props.elapsedMs).toBe(60_000)
    metadata.resolve({ duration: 60, size: 1_000_000, width: 720, height: 1280 })
    await flushPromises(60)
    expect(starts).toHaveLength(2)
    await vi.advanceTimersByTimeAsync(5_000)
    page.rerender()
    expect(findTrainingOverlay(page.element).props.elapsedMs).toBe(65_000)
    page.unmount()
    taroHarness.taroMock.createCameraContext.mockImplementation(originalCamera)
  })

  it('reserves the next adaptive segment before pausing and becomes manually resumable below 10MB', async () => {
    ...
    taroHarness.taroMock.getFileInfo
      .mockResolvedValueOnce({ size: 40 * 1024 * 1024 })
  ...
  it('stops both timers while buffer-paused and restarts them after manual continue', async () => {
    ...
    page.rerender()
    expect(findTrainingOverlay(page.element).props.elapsedMs).toBe(62_000)
  })
  ...
  it('keeps uploaded bytes occupied after deletion fails and retries cleanup without uploading again', async () => {
    ...
    expect(textContent(page.element)).toContain('网络较慢，训练已暂停')
    await vi.advanceTimersByTimeAsync(15000)
    await flushPromises(50)
    page.rerender()
    expect(taroHarness.unlinkMock).toHaveBeenCalledTimes(2)
    expect(apiMocks.uploadVideoSegment).toHaveBeenCalledTimes(1)
  ...
  it('deletes a local segment immediately when the pre-created server session status reports it uploaded', async () => {
    ...
    apiMocks.getVideoSessionStatus.mockResolvedValueOnce({
      video_id: 9,
      status: 'recording',
      uploaded_segments: [0]
    })
  ...
  it('keeps the pre-created server session for retry when recorder start fails', async () => {
    const timezoneOffset = vi.spyOn(Date.prototype, 'getTimezoneOffset').mockReturnValue(-480)
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-08-06T01:32:14Z'))
    const page = renderPage(ShoulderPressCameraPage)
    try {
      ...
      expect(taroHarness.storage.get(PENDING_SHOULDER_PRESS_SESSION_KEY)).toMatchObject({
        videoId: 9,
        trainingStartedAt: '2026-08-06T09:32:14+08:00',
        segments: []
      })
      page.rerender()
      expect(findTrainingOverlay(page.element).props.started).toBe(false)
  ...
  it('never constructs the recorder when pre-created session persistence fails', async () => {
    const timezoneOffset = vi.spyOn(Date.prototype, 'getTimezoneOffset').mockReturnValue(-480)
    vi.useFakeTimers()
    ...
    try {
      ...
      expect(findTrainingOverlay(page.element).props.started).toBe(false)
      expect(taroHarness.storage.has(PENDING_SHOULDER_PRESS_SESSION_KEY)).toBe(false)
      expect(textContent(page.element)).toContain('本地写入失败')
    } finally {
      page.unmount()
      timezoneOffset.mockRestore()
    }
  ...
  it('can retry pre-creation after local session persistence fails', async () => {
    const timezoneOffset = vi.spyOn(Date.prototype, 'getTimezoneOffset').mockReturnValue(-480)
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-08-06T01:32:14Z'))
    const page = renderPage(ShoulderPressCameraPage)
    try {
      ...
      expect(recorderHarness.instances).toHaveLength(0)
      expect(findButtonByText(page.element, '重试开始')).toBeTruthy()

      vi.setSystemTime(new Date('2026-08-06T01:35:00Z'))
      findButtonByText(page.element, '重试开始').props.onClick?.()
      await flushPromises()
  ...
  it.each([
    ...
    async (_caseName, key, durationMs, compressionState) => {
      const stored = {
        ...historicalPendingUpload(durationMs, compressionState),
        videoId: 9
      }
  ...
  it('requires cleanup and re-recording for a local-only historical session above 30 minutes', async () => {
    const stored = historicalPendingUpload(1_800_001, 'compression_failed')
    taroHarness.storage.set(PENDING_SHOULDER_PRESS_SESSION_KEY, stored)
    taroHarness.storage.set(LEGACY_PENDING_SHOULDER_PRESS_SESSION_KEY, stored)
    const page = renderPage(ShoulderPressUploadPage)
    ...
    expect(textContent(page.element)).toContain('历史长时录像没有可恢复的服务端会话')
    expect(taroHarness.taroMock.getFileInfo).not.toHaveBeenCalled()
    expect(apiMocks.createVideoSession).not.toHaveBeenCalled()
    expect(apiMocks.uploadVideoSegment).not.toHaveBeenCalled()
    expect(apiMocks.finalizeVideoSession).not.toHaveBeenCalled()

    clickButtonByText(page.element, '重新训练')
    await flushPromises()
    expect(taroHarness.storage.has(PENDING_SHOULDER_PRESS_SESSION_KEY)).toBe(false)
    expect(taroHarness.storage.has(LEGACY_PENDING_SHOULDER_PRESS_SESSION_KEY)).toBe(false)
    expect(taroHarness.unlinkMock).toHaveBeenCalledWith(expect.objectContaining({
      filePath: 'wxfile://store/historical-1800001.mp4'
    }))
  })
...
describe('审核演示模式', () => {
  it('启动检查中不显示绑定码输入', () => {
    ...
    expect(textContent(page.element)).toContain('正在恢复登录')
    expect(textContent(page.element)).toContain('正在检查登录状态')
    expect(findAll(page.element, (element) => element.type === 'Input')).toHaveLength(0)
  })

  it('只有后端明确 unbound 后才显示绑定码', async () => {
    taroHarness.taroMock.login.mockResolvedValueOnce({ code: 'startup-code' })
    requestMock.mockResolvedValueOnce({ status: 'unbound' })
    const page = renderPage(BindPage)
  ...
  it('清缓存后按微信身份恢复并保存新 token', async () => {
    taroHarness.taroMock.login.mockResolvedValueOnce({ code: 'startup-code' })
    requestMock.mockResolvedValueOnce({
  ...
  it('有效旧 token 静默迁移时保留原 token', async () => {
    setPatientAppToken('legacy-token')
    taroHarness.taroMock.login.mockResolvedValueOnce({ code: 'startup-code' })
    requestMock.mockResolvedValueOnce({
      status: 'authenticated',
      token: null,
    ...
    expect(requestMock).toHaveBeenCalledWith('/patient-app/wechat-session/', {
      method: 'POST',
      data: { wx_code: 'startup-code' }
    })
    expect(getPatientAppToken()).toBe('legacy-token')
    expect(taroHarness.taroMock.redirectTo).toHaveBeenCalledWith({ url: '/pages/home/index' })
  })
  ...
  it('登录检查失败只显示重试且不显示绑定输入', async () => {
    ...
    expect(textContent(page.element)).toContain('网络连接失败，请稍后重试')
    expect(findButtonByText(page.element, '重新检查登录')).toBeTruthy()
    expect(findAll(page.element, (element) => element.type === 'Input')).toHaveLength(0)
  })

  it('点击重新检查后可从 error 进入 unbound', async () => {
    taroHarness.taroMock.login
      .mockResolvedValueOnce({ code: 'failed-startup-code' })
      .mockResolvedValueOnce({ code: 'retry-startup-code' })
    requestMock
    ...
    await flushPromises()
    page.rerender()
    clickButtonByText(page.element, '重新检查登录')
    await flushPromises()
    page.rerender()

    expect(taroHarness.taroMock.login).toHaveBeenCalledTimes(2)
    expect(findAll(page.element, (element) => element.type === 'Input')).toHaveLength(1)
  })

  it('普通绑定启动和提交使用两个不同微信 code', async () => {
    taroHarness.taroMock.login
      .mockResolvedValueOnce({ code: 'startup-code' })
      .mockResolvedValueOnce({ code: 'binding-code-login' })
    requestMock.mockResolvedValueOnce({ status: 'unbound' }).mockResolvedValueOnce({
      token: 'real-token',
      project_patient_id: 1,
      patient: { id: 1, name: '王阿姨' },
      project: { id: 1, name: '居家运动项目' }
  ...
  it('页面卸载后忽略迟到的恢复响应', async () => {
    ...
    expect(taroHarness.taroMock.redirectTo).not.toHaveBeenCalled()
  })
  ...
  it('示范视频刷新失败不触碰摄像上下文、录像、上传或本地会话', async () => {
    session.startDemoSession()
    taroHarness.routerParams.actionId = '888808'
    const page = renderPage(ShoulderPressCameraPage)
    await flushPromises(20)
    page.rerender()
    const requestsBeforeRefresh = publicRequestMock.mock.calls.length
    publicRequestMock.mockRejectedValueOnce(new Error('manifest unavailable'))

    const refresh = findTrainingOverlay(page.element).props.onVideoError
    if (!refresh) throw new Error('示范视频刷新回调缺失')
    await expect(refresh()).rejects.toThrow('示范视频暂时无法播放')
    await flushPromises(20)

    expect(publicRequestMock.mock.calls.length).toBe(requestsBeforeRefresh + 1)
  ...
  it.each(DEMO_MOTION_ACTION_CASES.filter(action => action.id === 888808))(
  ...
  it('演示高抬腿切到后台时暂停计时并在返回后继续', async () => {
    session.startDemoSession()
    taroHarness.routerParams.actionId = '888808'
    vi.useFakeTimers()
...
describe('计数组运动页面', () => {
  ...
  it('演示组到五分钟自动进入休息，不创建真实录像或上传', async () => {
    vi.useFakeTimers()
    const action = { ...PRESCRIPTION!.actions[0], dose_mode: 'sets' as const, repetitions: 10, sets: 2, count_unit: 'total' as const }
    const page = renderPage(() => <CountedCamera action={action} demo />)
    clickButtonByText(page.element, '开始本组'); await flushPromises(); page.rerender()
    await vi.advanceTimersByTimeAsync(300000); await flushPromises(); page.rerender()
    expect(textContent(page.element)).toContain('组间休息')
  ...
  it('演示逐组手动完成，三分钟休息不可跳过且结束不自动开始', async () => {
    ...
    await flushPromises()
    page.rerender()
    expect(textContent(page.element)).toContain('每侧 10 个')
    clickButtonByText(page.element, '开始本组')
    await flushPromises()
    page.rerender()
    ...
    page.rerender()
    expect(textContent(page.element)).toContain('03:00')
    expect(findButtonByText(page.element, '开始下一组').props.disabled).toBe(true)
    await vi.advanceTimersByTimeAsync(180000)
    page.rerender()
    expect(findButtonByText(page.element, '开始下一组').props.disabled).toBe(false)
    expect(textContent(page.element)).not.toContain('完成本组')
    clickButtonByText(page.element, '开始下一组')
    await flushPromises()
    page.rerender()
    clickButtonByText(page.element, '完成本组')
    await flushPromises()
    page.rerender()
    expect(textContent(page.element)).toContain('演示运动已做完')
    expect(taroHarness.taroMock.createCameraContext).not.toHaveBeenCalled()
    expect(taroHarness.taroMock.saveFile).not.toHaveBeenCalled()
...
describe('计数组真实录像', () => {
  beforeEach(() => { vi.spyOn(session, 'isDemoSession').mockReturnValue(false) })
  afterEach(() => { vi.mocked(session.isDemoSession).mockRestore() })
  it('安全保存后网络失败仍可进入休息，重启保留已完成组且片段未提前删除', async () => {
    vi.useFakeTimers()
    setPatientAppToken('counted-user')
    const action = { ...PRESCRIPTION!.actions[0], dose_mode: 'sets' as const, repetitions: 10, sets: 2, count_unit: 'total' as const }
    requestMock.mockImplementation(async (path: string) => {
      if (path === '/patient-app/home/') return { project_patient_id: 18, current_prescription: { ...PRESCRIPTION, actions: [action] } }
      if (path === '/patient-app/current-prescription/') return { ...PRESCRIPTION, actions: [action] }
      if (path === '/patient-app/motion-sessions/') return { id: 90 }
    ...
    expect(taroHarness.taroMock.createCameraContext.mock.calls).toHaveLength(1)
    await vi.advanceTimersByTimeAsync(5000)
    clickButtonByText(page.element, '完成本组')
    await flushPromises(40); page.rerender()
    expect(textContent(page.element)).toContain('03:00')
    expect(taroHarness.removeSavedFileMock).not.toHaveBeenCalled()
    page.unmount(); reactHarness.reset()
    const restored = renderPage(() => <CountedCamera action={action} demo={false} />)
...
it('休息按墙钟播报三十秒和末五秒，切后台回来不补播过时倒数', async () => {
  vi.useFakeTimers()
  const action = { ...PRESCRIPTION!.actions[0], dose_mode: 'sets' as const, repetitions: 10, sets: 2, count_unit: 'total' as const }
  const page = renderPage(() => <CountedCamera action={action} demo />)
  clickButtonByText(page.element, '开始本组'); await flushPromises(); page.rerender()
  clickButtonByText(page.element, '完成本组'); await flushPromises(50); page.rerender()
  expect(motionInstructionAudioHarness.play.mock.calls.some(([src]) => String(src).includes('motion-rest-start'))).toBe(true)
  await vi.advanceTimersByTimeAsync(150000); page.rerender(); await flushPromises()
  expect(motionInstructionAudioHarness.play.mock.calls.some(([src]) => String(src).includes('motion-rest-thirty'))).toBe(true)
  taroHarness.hideCallbacks.at(-1)?.()
  motionInstructionAudioHarness.play.mockClear()
  vi.setSystemTime(Date.now() + 29000)
  taroHarness.showCallbacks.at(-1)?.(); page.rerender(); await flushPromises()
  expect(motionInstructionAudioHarness.play).not.toHaveBeenCalled()
  await vi.advanceTimersByTimeAsync(1000); page.rerender(); await flushPromises()
  expect(motionInstructionAudioHarness.play.mock.calls.some(([src]) => String(src).includes('motion-rest-ready'))).toBe(true)
  expect(findButtonByText(page.element, '开始下一组').props.disabled).toBe(false)
  page.unmount()
})


it.each(DEMO_MOTION_ACTION_CASES.filter(action => action.id !== 888808))('演示计数组动作 $name 从正式入口进入手动完成流程', async ({ id }) => {
  session.startDemoSession()
  taroHarness.routerParams.actionId = String(id)
  const page = renderPage(ShoulderPressCameraPage)
  await flushPromises(30); page.rerender(); await flushPromises(); page.rerender()
  expect(textContent(page.element)).toContain('共 3 组')
  clickButtonByText(page.element, '开始本组'); await flushPromises(); page.rerender()
...
it('计数组历史按一次运动显示完成组与视频状态，不将三组显示成三次', async () => {
  requestMock.mockResolvedValue({ prescription_action: 42, last_7_days_completed_count: 1, last_30_days_completed_count: 1, records: [], motion_sessions: [{ id: 90, training_date: '2026-09-16', planned_sets: 3, completed_sets: 3, uploaded_sets: 2, status: 'awaiting_upload' }] })
  const page = renderPage(ActionHistoryPage)
  await taroHarness.showCallbacks[0](); await flushPromises(); page.rerender()
  expect(textContent(page.element)).toContain('运动已做完，视频待上传')
  expect(textContent(page.element)).toContain('已做完 3/3 组')
  expect(textContent(page.element)).toContain('视频已上传 2/3 组')
  expect(textContent(page.element)).not.toContain('暂无训练记录')
})

describe('计数组录像恢复与上传边界', () => {
  const action = { ...PRESCRIPTION!.actions[0], dose_mode: 'sets' as const, repetitions: 10, sets: 2, count_unit: 'total' as const }
  beforeEach(() => {
    directUploadMock.mockReset(); directUploadMock.mockResolvedValue(undefined)
    vi.spyOn(session, 'isDemoSession').mockReturnValue(false)
    vi.useFakeTimers()
    setPatientAppToken('counted-18')
    requestMock.mockImplementation(async (path: string) => {
      if (path === '/patient-app/home/') return { project_patient_id: 18 }
      if (path === '/patient-app/current-prescription/') return { ...PRESCRIPTION, actions: [action] }
      if (path === '/patient-app/motion-sessions/recover/') return { id: 90 }
      if (path === '/patient-app/training-video-direct-uploads/') return { video_id: 9, status: 'recording', upload_url: 'https://upload.example.test', upload_token: 'scoped', object_key: 'whole.mp4' }
      if (path.includes('/training-video-direct-uploads/')) return { video_id: 9, status: 'recording' }
  ...
  async function firstGroup(page: RenderedPage) {
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    await vi.advanceTimersByTimeAsync(5000)
    clickButtonByText(page.element, '完成本组'); await flushPromises(50); page.rerender()
  }
  it('每组安全保存后重建摄像头，旧实例回调不能解锁或中断下一组', async () => {
    const page = await open()
    const oldCamera = findAll(page.element, element => element.type === 'Camera')[0]
    await firstGroup(page)
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    expect(findButtonByText(page.element, '开始下一组').props.disabled).toBe(true)
    oldCamera.props.onInitDone?.(); page.rerender()
    expect(findButtonByText(page.element, '开始下一组').props.disabled).toBe(true)
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(40); page.rerender()
    expect(taroHarness.taroMock.createCameraContext).toHaveBeenCalledTimes(2)
    ;(oldCamera.props.onStop as (() => void))(); page.rerender()
    expect(textContent(page.element)).toContain('完成本组')
    page.unmount()
  })

  it('点击完成立即进入休息，收到整段视频前不播放休息音频或开始下一组', async () => {
    recorderHarness.useReal = true
    let stop: any
    taroHarness.taroMock.createCameraContext.mockReturnValueOnce({ startRecord: vi.fn(o => o.success()), stopRecord: vi.fn(o => { stop = o }) })
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    ...
    stop.success({ tempVideoPath: 'wxfile://whole.mp4' }); await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('02:48')
    expect(textContent(page.element)).toContain('已做完 1/2 组')
    const saved = (taroHarness.taroMock.getStorageSync('motioncare.countedMotionQueue.v1') as any)[18][0].attempts[0]
    expect(saved).toMatchObject({ uploadMode: 'direct', completionReason: 'manual', video: { durationMs: 10000 } })
    expect(saved.segments).toBeUndefined()
    page.unmount()
  })
  it('五分钟静默进入休息并等待 SDK 文件，不重复停止或显示提前上限警告', async () => {
    recorderHarness.useReal = true
    let start: any
    const camera = { startRecord: vi.fn(o => { start = o; o.success() }), stopRecord: vi.fn() }
    taroHarness.taroMock.createCameraContext.mockReturnValueOnce(camera)
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    expect(start.timeout).toBe(300)
    await vi.advanceTimersByTimeAsync(299000); page.rerender()
    expect(camera.stopRecord).not.toHaveBeenCalled()
    expect(textContent(page.element)).not.toContain('即将达到')
    await vi.advanceTimersByTimeAsync(1000); page.rerender()
    expect(textContent(page.element)).toContain('组间休息')
    expect(camera.stopRecord).not.toHaveBeenCalled()
    start.timeoutCallback({ tempVideoPath: 'wxfile://five-minutes.mp4' })
    await flushPromises(80); page.rerender()
    const saved = (taroHarness.taroMock.getStorageSync('motioncare.countedMotionQueue.v1') as any)[18][0].attempts[0]
    expect(saved).toMatchObject({ completed: true, completionReason: 'time_limit', video: { durationMs: 300000 } })
    expect(Date.parse(saved.endedAt) - Date.parse(saved.startedAt)).toBe(300000)
    expect(apiMocks.uploadVideoSegment).not.toHaveBeenCalled()
    page.unmount()
  })
  it('尾段永久失联后必须重建摄像头重做，迟到结果不能保存或完成新组', async () => {
    recorderHarness.useReal = true
    let oldStop: any
    const camera = {
      startRecord: vi.fn(options => options.success()),
      stopRecord: vi.fn(options => { oldStop = options })
    }
    taroHarness.taroMock.createCameraContext.mockReturnValueOnce(camera)
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    await vi.advanceTimersByTimeAsync(5000)
    clickButtonByText(page.element, '完成本组'); await flushPromises(); page.rerender()
    await vi.advanceTimersByTimeAsync(30000); page.rerender()
    expect(textContent(page.element)).toContain('摄像头返回录像超时，请重做本组')
    expect(textContent(page.element)).not.toContain('重试保存本组')
    ...
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    expect(taroHarness.taroMock.createCameraContext).toHaveBeenCalledTimes(2)
    oldStop.success({ tempVideoPath: 'wxfile://late-old-group.mp4' })
    await flushPromises(40); page.rerender()
    expect(taroHarness.taroMock.saveFile).not.toHaveBeenCalled()
    expect(textContent(page.element)).not.toContain('已做完 1/2 组')
    expect(textContent(page.element)).toContain('完成本组')
  ...
  it('上传中显示百分比和速度，传完进入确认状态而非失败状态', async () => {
    ...
    directUploadMock.mockImplementationOnce(input => { progress = percent => input.onProgress(percent, 19_876_543 * percent / 100); return upload.promise })
    ...
    requestMock.mockImplementation(async path => path.includes('/recover/') ? { id: 90 } : { video_id: 9, status: 'attached' })
  ...
  it.each(['expired', 'failed'])('分片收齐或排队不清理本地录像，%s 重建上传仍沿用原组尝试，绑定后才清理', async (status) => {
    const page = await open(); await firstGroup(page)
    const queue = taroHarness.taroMock.getStorageSync('motioncare.countedMotionQueue.v1') as any
    const attempt = queue[18][0].attempts[0]
    attempt.segments = [attempt.video]; delete attempt.video; delete attempt.videoId; delete attempt.uploadMode
    taroHarness.taroMock.setStorageSync('motioncare.countedMotionQueue.v1', queue)
    clickButtonByText(page.element, '重试上传'); await flushPromises(80); page.rerender()
    expect(apiMocks.createVideoSession).toHaveBeenCalledTimes(1)
    ...
    expect(renewed.motionAttemptId).toBe(original.motionAttemptId)
    expect(renewed.clientSessionId).not.toBe(original.clientSessionId)
    expect(taroHarness.removeSavedFileMock).not.toHaveBeenCalled()
    apiMocks.getVideoSessionStatus.mockResolvedValueOnce({ video_id: 9, status: 'attached', uploaded_segments: [0] })
    clickButtonByText(page.element, '重试上传'); await flushPromises(50); page.rerender()
    expect(taroHarness.removeSavedFileMock).toHaveBeenCalledTimes(1)
  ...
  it('切后台中断当前组清理录像，回来仍从同组重新开始且中断内容不上传', async () => {
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    ...
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    expect(taroHarness.taroMock.createCameraContext.mock.calls).toHaveLength(2)
    page.unmount()
  })
  it('已保存列表超过旧阈值仍可开始下一组，未上传第一组保持保护', async () => {
    const page = await open(); await firstGroup(page)
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    taroHarness.getSavedFileListMock.mockImplementationOnce(options => options.success({ fileList: [{ filePath: 'wxfile://kept', size: 36 * 1024 * 1024 }] }))
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    expect(textContent(page.element)).toContain('完成本组')
    expect(taroHarness.taroMock.createCameraContext).toHaveBeenCalledTimes(2)
    expect(taroHarness.taroMock.createCameraContext.mock.results[0].value.startRecord).toHaveBeenCalledTimes(1)
    expect(taroHarness.taroMock.createCameraContext.mock.results[1].value.startRecord).toHaveBeenCalledTimes(1)
    expect(taroHarness.removeSavedFileMock).not.toHaveBeenCalled()
    page.unmount()
  })
  it('空间不足留下临时录像时先补传，云端确认后才允许开始下一组', async () => {
    taroHarness.taroMock.saveFile.mockRejectedValueOnce(new Error('storage full'))
    directUploadMock.mockRejectedValue(new Error('network unavailable'))
    const page = await open(); await firstGroup(page)
    expect(textContent(page.element)).toContain('本机空间不足')
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(40); page.rerender()
    expect(textContent(page.element)).toContain('请先完成临时录像上传，再开始下一组')
    expect(taroHarness.taroMock.createCameraContext.mock.results[0].value.startRecord).toHaveBeenCalledTimes(1)
    requestMock.mockImplementation(async path => path.includes('/recover/') ? { id: 90 }
      : path.includes('/current-prescription/') ? { ...PRESCRIPTION, actions: [action] } : { video_id: 9, status: 'attached' })
    clickButtonByText(page.element, '重试上传'); await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('视频已上传 1/1 组')
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(40); page.rerender()
  ...
  it('新组前发现运动计划已更新则阻止继续原计划', async () => {
    const page = await open(); await firstGroup(page)
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    requestMock.mockImplementation(async path => path === '/patient-app/current-prescription/' ? { ...PRESCRIPTION, actions: [] } : { id: 90 })
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    expect(textContent(page.element)).toContain('运动计划已更新')
    expect(taroHarness.taroMock.createCameraContext.mock.calls).toHaveLength(1)
    page.unmount()
  })
  it('完成失败后重试尚未返回时重做本组，仍可退出并重新开始且保留前两组', async () => {
    ...
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    const oldCamera = findAll(page.element, element => element.type === 'Camera')[0]
    taroHarness.taroMock.getFileInfo.mockRejectedValueOnce(new Error('读取录像失败'))
    await vi.advanceTimersByTimeAsync(1)
    clickButtonByText(page.element, '完成本组'); await flushPromises(30); page.rerender()
    const stop = deferred<never>()
    taroHarness.taroMock.getFileInfo.mockReturnValueOnce(stop.promise)
    ...
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    expect.soft(taroHarness.taroMock.createCameraContext.mock.calls).toHaveLength(4)
  ...
  it.each([4900, 40000])('真实录像控制器完成本组（%s 毫秒）显示保存等待且不报停止失败', async timing => {
    recorderHarness.useReal = true
    ...
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    await vi.advanceTimersByTimeAsync(timing)
    clickButtonByText(page.element, '完成本组')
    ...
    stops[0].success({ tempVideoPath: 'wxfile://temp/whole.mp4' })
    await flushPromises(80); page.rerender()
  ...
  it('最后一组等待完整视频时显示保存状态，不显示上一轮休息或提前完成', async () => {
    const page = await open(); await firstGroup(page)
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    await vi.advanceTimersByTimeAsync(10000)
    let stop: any
    taroHarness.taroMock.createCameraContext.mock.results.at(-1)!.value.stopRecord.mockImplementationOnce((o: any) => { stop = o })
    clickButtonByText(page.element, '完成本组'); await flushPromises(30); page.rerender()
    expect(textContent(page.element)).toContain('正在保存本组')
    expect(findAll(page.element, node => node.props?.className === 'counted-rest')).toHaveLength(0)
    expect(textContent(page.element)).not.toContain('本次运动已完成')
    stop.success({ tempVideoPath: 'wxfile://last-whole.mp4' }); await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('运动已做完，视频待上传')
    page.unmount()
  })
  ...
  it('开始前检查期间摄像头失效，不应继续对失效摄像头启动录像', async () => {
    ...
    expect(taroHarness.taroMock.createCameraContext.mock.calls).toHaveLength(0)
    expect(findButtonByText(page.element, '返回运动计划').props.disabled).toBe(false)
    expect(textContent(page.element)).toContain('重新开启摄像头')
    page.unmount()
  })
  it('停止超过十秒后返回视频，仍保存上传原组且不会重复停止录像', async () => {
    recorderHarness.useReal = true
    let stop: any
    const camera = {
      startRecord: vi.fn(options => options.success()),
      stopRecord: vi.fn(options => { stop = options })
    }
    taroHarness.taroMock.createCameraContext.mockReturnValueOnce(camera)
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    ...
    expect(diagnostic.environment).toEqual({ platform: 'ios', system: 'iOS 26.6.1', model: 'iPhone 13', version: '8.0.78', SDKVersion: '3.17.0' })
    ...
    clickButtonByText(page.element, '重试保存本组'); await flushPromises(); page.rerender()
    expect(findButtonByText(page.element, '返回运动计划').props.disabled).toBe(false)
    stop.success({ tempVideoPath: 'wxfile://late-tail.mp4' })
    await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('已做完 1/2 组')
    expect(textContent(page.element)).toContain('02:48')
    expect(camera.stopRecord).toHaveBeenCalledTimes(1)
    expect(directUploadMock).toHaveBeenCalledTimes(1)
    expect(apiMocks.uploadVideoSegment).not.toHaveBeenCalled()
    expect(textContent(page.element)).not.toContain('仍在等待保存')
    page.unmount()
  })
  it('真实设备明确停止失败时结束 loading，不计完成并保留脱敏诊断', async () => {
    recorderHarness.useReal = true
    ...
    await vi.advanceTimersByTimeAsync(1)
    clickButtonByText(page.element, '完成本组'); await flushPromises(); page.rerender()
    expect(findButtonByText(page.element, '重做本组').props.disabled).not.toBe(true)
    expect(textContent(page.element)).toContain('摄像头未能结束录像')
    expect(textContent(page.element)).not.toContain('重试保存本组')
    expect(textContent(page.element)).not.toContain('已做完 1/2 组')
    expect(apiMocks.createVideoSession).not.toHaveBeenCalled()
    ...
    expect(diagnostics).toEqual(expect.objectContaining({ events: expect.arrayContaining([
      expect.objectContaining({ stage: 'recording', error_code: 'recording_native_1001' })
    ]) }))
    clickButtonByText(page.element, '重做本组'); await flushPromises(); page.rerender()
    expect(findButtonByText(page.element, '返回运动计划').props.disabled).toBe(false)
    page.unmount()
  })
  it('整段保存失败仍显示休息但不能提交完成声明', async () => {
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    taroHarness.taroMock.getFileInfo.mockRejectedValueOnce(new Error('整段读取失败'))
    await vi.advanceTimersByTimeAsync(1)
    clickButtonByText(page.element, '完成本组'); await flushPromises(30); page.rerender()
  ...
  it('整段元数据读取失败后原文件重试，不重复停止且只完成一次', async () => {
    recorderHarness.useReal = true
    const camera = {
      startRecord: vi.fn(options => options.success()),
      stopRecord: vi.fn(options => options.success({ tempVideoPath: 'wxfile://temp/retried-final.mp4' }))
    }
    taroHarness.taroMock.createCameraContext.mockReturnValueOnce(camera)
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    taroHarness.taroMock.getFileInfo.mockRejectedValueOnce(new Error('文件保存失败'))
    await vi.advanceTimersByTimeAsync(1)
    clickButtonByText(page.element, '完成本组'); await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('重试保存本组')
    expect(apiMocks.createVideoSession).not.toHaveBeenCalled()
    clickButtonByText(page.element, '重试保存本组'); await flushPromises(80); page.rerender()
    expect(textContent(page.element)).toContain('已做完 1/2 组')
    expect(textContent(page.element)).toContain('03:00')
    expect(textContent(page.element)).not.toContain('重做本组')
    expect(camera.stopRecord).toHaveBeenCalledTimes(1)
  ...
  it('尾段已保存但读取元数据失败后重做，也清理尚未登记的持久文件', async () => {
    recorderHarness.useReal = true
    const page = await open()
    clickButtonByText(page.element, '开始本组'); await flushPromises(30); page.rerender()
    taroHarness.taroMock.saveFile.mockResolvedValueOnce({ savedFilePath: 'wxfile://saved/unlisted.mp4' })
    taroHarness.taroMock.getFileInfo.mockRejectedValueOnce(new Error('文件信息读取失败'))
    clickButtonByText(page.element, '完成本组'); await flushPromises(80); page.rerender()
    clickButtonByText(page.element, '重做本组'); await flushPromises(40); page.rerender()
    expect(taroHarness.removeSavedFileMock).toHaveBeenCalledWith(expect.objectContaining({ filePath: 'wxfile://saved/unlisted.mp4' }))
    expect(apiMocks.createVideoSession).not.toHaveBeenCalled()
    page.unmount()
  })
  ...
  it('连续跨上海午夜仍归首次日期，全部组做完后隔天仅补传也不新建运动', async () => {
    vi.setSystemTime(new Date('2026-09-16T15:59:50.000Z'))
    const page = await open(); await firstGroup(page)
    await vi.advanceTimersByTimeAsync(180000); page.rerender()
    initializeCamera(page.element); page.rerender()
    clickButtonByText(page.element, '开始下一组'); await flushPromises(30); page.rerender()
    ...
    clickButtonByText(page.element, '完成本组'); await flushPromises(50); page.rerender()
    expect(textContent(page.element)).toContain('运动已做完，视频待上传')
    expect(apiMocks.createVideoSession.mock.calls.every(([input]) => input.trainingDate === '2026-09-16')).toBe(true)
    const recoverCalls = requestMock.mock.calls.filter(([path]) => path === '/patient-app/motion-sessions/recover/')
    expect(recoverCalls.at(-1)![1].data.completed_sets.map((item: { index: number }) => item.index)).toEqual([1, 2])
    page.unmount(); reactHarness.reset()
    vi.setSystemTime(new Date('2026-09-18T01:00:00.000Z'))
    const recovered = renderPage(() => <CountedCamera action={action} demo={false} />)
    await flushPromises(40); recovered.rerender()
    expect(textContent(recovered.element)).toContain('运动已做完，视频待上传')
    expect(textContent(recovered.element)).not.toContain('开始本组')
    recovered.unmount()
  })
  it('部分完成退出后跨上海日期返回从第一组新开，旧组只补传', async () => {
    vi.setSystemTime(new Date('2026-09-16T15:59:50.000Z'))
    const page = await open(); await firstGroup(page)
    taroHarness.hideCallbacks.at(-1)?.(); await flushPromises()
    vi.setSystemTime(new Date('2026-09-16T16:03:00.000Z'))
    taroHarness.showCallbacks.at(-1)?.(); await flushPromises(40); page.rerender()
    expect(textContent(page.element)).toContain('第 1/2 组')
    expect(textContent(page.element)).toContain('已进入新的一天')
    page.unmount()
  })
  it('账号切换后不恢复或上传其他用户的已完成组', async () => {
    const page = await open(); await firstGroup(page); page.unmount(); reactHarness.reset()
    setPatientAppToken('counted-22')
    requestMock.mockImplementation(async path => path === '/patient-app/home/' ? { project_patient_id: 22 } : { ...PRESCRIPTION, actions: [action] })
    apiMocks.createVideoSession.mockClear(); apiMocks.getVideoSessionStatus.mockClear()
    const another = await open()
    expect(textContent(another.element)).toContain('第 1/2 组')
    expect(apiMocks.getVideoSessionStatus).not.toHaveBeenCalled()
    expect(apiMocks.createVideoSession).not.toHaveBeenCalled()
    another.unmount()
  })
